    ...
    stages {
        ...
        stage('Run Container on EC2-INSTANCE'){
            steps {
                ...
                sh "ssh -o StrictHostKeyChecking=no ec2-user@35.154.147.149 'docker run -p 4000:4000 -d jagadeeshsiriveru/webimg:v1'"
                }
                
                sshagent(['EC2-INSTANCE']) {
                sh "ssh -o StrictHostKeyChecking=no ec2-user@15.207.18.236 'docker run -p 4000:4000 -d jagadeeshsiriveru/webimg:v1'"
                  }
            }
        }
    }
